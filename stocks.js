...
async function grab() {
  for (const singleticker of tickers) {
    ...
    if (count % 250 == 0 && count > 1) {
      ...
      process.stdout.write(
        `Rate: ${parseInt(count / ((endTime - startTime) / 1000), 10)}x`
      );
    }
    if (!quote) {
      return;
    ...
    let xposition =
      7 + Math.floor(tickers.indexOf(singleticker) / rows) * columnwidth;
    readline.cursorTo(
      process.stdout,
      xposition,
      tickers.indexOf(singleticker) % rows
    );
    process.stdout.write(
      `\x1b[37m${quote.price.toFixed(2)}${
